...
class Sky:
    def __init__(self, horizon, style='level'):
        ...
        if self.style == 'overworld':
            ...
            sky_surfaces = import_folder('../graphics/overworld/clouds')
            self.skies = []
            for surface in [choice(sky_surfaces) for image in range(10)]:
                x = randint(0, screen_width)
                y = randint(0, screen_height / 2)
                rect = surface.get_rect(midbottom=(x, y))
                self.skies.append((surface, rect))
...
class Clouds:
    def __init__(self, horizon, leve_width, cloud_number):
        cloud_surf_list = import_folder('../graphics/decoration/clouds')
        min_x = -screen_width
        max_x = leve_width + screen_width
        min_y = 0
        max_y = horizon
